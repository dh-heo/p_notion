...
| `PNOTION_API_TOKEN` | 설정 시 `Authorization: Bearer <토큰>`로 API 호출 허용(에이전트/자동화용). 미설정이면 토큰 인증 비활성(쿠키 전용). 비밀번호와 **동급 권한**이니 비밀로 관리 |

## API (에이전트 / 자동화)

브라우저 없이 외부 프로그램(예: AI 에이전트)이 페이지를 만들고 글을 쓸 수 있습니다. 모든 데이터 라우트는 인증 게이트 뒤에 있으며, 세션 쿠키 **또는** Bearer 토큰으로 인증합니다.

**1. 토큰 발급/설정** — 서버 기동 시 `PNOTION_API_TOKEN`만 주면 됩니다.

```bash
PNOTION_API_TOKEN=$(openssl rand -hex 32) npm start
```

**2. 마크다운으로 페이지 생성** — `POST /api/ingest`가 마크다운을 블록으로 변환해 새 페이지를 만듭니다(가장 간단·안전한 경로).

```bash
curl -X POST http://<HOST>/api/ingest \
  -H "Authorization: Bearer <토큰>" \
  -H "Content-Type: application/json" \
  -d '{"title":"회의록","markdown":"# 제목\n\n본문 **굵게**\n\n- 항목\n- [ ] 할 일\n\n```js\nconsole.log(1)\n```"}'
# → { "page": { "id": "...", ... }, "block_count": 6 }
```

| 입력 | 설명 |
|---|---|
| `markdown` | (필수) 변환할 마크다운. `#/##/###`→제목, `-/*/+`→불릿(2칸 들여쓰기=1단계), `- [ ]/- [x]`→할 일, ` ``` `→코드, `>`→인용, `---`→구분선, 그 외→문단. 인라인 `**굵게**`·`*기울임*`·`` `코드` ``·`[링크](url)` 지원 |
| `title` | (선택) 페이지 제목 |
| `parent_page_id` | (선택) 하위 페이지로 만들 부모 id |

블록 단위로 세밀하게 다루려면 같은 토큰으로 `POST /api/pages`, `POST /api/blocks`를 그대로 쓰면 됩니다.

> **http 주의:** http로 서비스 중이면 토큰·본문이 평문으로 전송됩니다. 보안그룹을 에이전트 출발 IP로 제한하거나 앞단에 HTTPS를 두세요. 도메인이 없어도 `nip.io`/`sslip.io` 호스트네임으로 Let's Encrypt 인증서를 받을 수 있습니다.
...
│   └── routes/        # pages / blocks / upload / auth / ingest
